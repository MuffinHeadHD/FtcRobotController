...
@TeleOp(name = "Blue Close NEAF")
class CLoseNEAFBlue : LinearOpMode() {
    ...
    runBlocking(
        RaceAction(
            ...
            drive.actionBuilder(launchPosition)
                ...
                .lineToY(-60.0)
                .stopAndAdd(SleepAction(3.0))
                .stopAndAdd(robot.SpindexerRotate(-1))
                .stopAndAdd(SleepAction(2.0))
                .stopAndAdd(robot.SpindexerRotate(-1))
                .stopAndAdd(SleepAction(2.0))
                .stopAndAdd(robot.SpindexerRotate(-1))
                .stopAndAdd(SleepAction(2.0))
                .lineToY(-50.0)
                .splineToLinearHeading(launchPosition, -(2 * PI / 2))
                .stopAndAdd(robot.SetIntakeIn())
                .build()
        )
    )
